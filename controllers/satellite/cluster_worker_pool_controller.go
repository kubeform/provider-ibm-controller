...
// ClusterWorkerPoolReconciler reconciles a ClusterWorkerPool object
type ClusterWorkerPoolReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=satellite.ibm.kubeform.com,resources=clusterworkerpools,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=satellite.ibm.kubeform.com,resources=clusterworkerpools/status,verbs=get;update;patch

func (r *ClusterWorkerPoolReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("clusterworkerpool", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ClusterWorkerPool")
...
func (r *ClusterWorkerPoolReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &satellitev1alpha1.ClusterWorkerPool{}); err != nil {
			klog.Error(err, "unable to set up auditor", satellitev1alpha1.ClusterWorkerPool{}.APIVersion, satellitev1alpha1.ClusterWorkerPool{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&satellitev1alpha1.ClusterWorkerPool{}).
