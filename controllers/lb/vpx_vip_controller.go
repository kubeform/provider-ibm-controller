...
// VpxVipReconciler reconciles a VpxVip object
type VpxVipReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=lb.ibm.kubeform.com,resources=vpxvips,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=lb.ibm.kubeform.com,resources=vpxvips/status,verbs=get;update;patch

func (r *VpxVipReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vpxvip", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VpxVip")
...
func (r *VpxVipReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &lbv1alpha1.VpxVip{}); err != nil {
			klog.Error(err, "unable to set up auditor", lbv1alpha1.VpxVip{}.APIVersion, lbv1alpha1.VpxVip{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&lbv1alpha1.VpxVip{}).
