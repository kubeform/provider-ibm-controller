...
// +kubebuilder:rbac:groups=lb.ibm.kubeform.com,resources=servicegroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=lb.ibm.kubeform.com,resources=servicegroups/status,verbs=get;update;patch

func (r *ServiceGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("servicegroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServiceGroup")
...
func (r *ServiceGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &lbv1alpha1.ServiceGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", lbv1alpha1.ServiceGroup{}.APIVersion, lbv1alpha1.ServiceGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&lbv1alpha1.ServiceGroup{}).
