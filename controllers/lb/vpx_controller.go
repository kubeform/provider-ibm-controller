...
// VpxReconciler reconciles a Vpx object
type VpxReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=lb.ibm.kubeform.com,resources=vpxes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=lb.ibm.kubeform.com,resources=vpxes/status,verbs=get;update;patch

func (r *VpxReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vpx", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Vpx")
...
func (r *VpxReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &lbv1alpha1.Vpx{}); err != nil {
			klog.Error(err, "unable to set up auditor", lbv1alpha1.Vpx{}.APIVersion, lbv1alpha1.Vpx{}.Kind)
			return err
		}
	}
