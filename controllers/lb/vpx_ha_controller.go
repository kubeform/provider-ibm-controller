...
// VpxHaReconciler reconciles a VpxHa object
type VpxHaReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=lb.ibm.kubeform.com,resources=vpxhas,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=lb.ibm.kubeform.com,resources=vpxhas/status,verbs=get;update;patch

func (r *VpxHaReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vpxha", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VpxHa")
...
func (r *VpxHaReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &lbv1alpha1.VpxHa{}); err != nil {
			klog.Error(err, "unable to set up auditor", lbv1alpha1.VpxHa{}.APIVersion, lbv1alpha1.VpxHa{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&lbv1alpha1.VpxHa{}).
