...
// EndpointSubscriptionReconciler reconciles a EndpointSubscription object
type EndpointSubscriptionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=apigateway.ibm.kubeform.com,resources=endpointsubscriptions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apigateway.ibm.kubeform.com,resources=endpointsubscriptions/status,verbs=get;update;patch

func (r *EndpointSubscriptionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("endpointsubscription", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EndpointSubscription")
...
func (r *EndpointSubscriptionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apigatewayv1alpha1.EndpointSubscription{}); err != nil {
			klog.Error(err, "unable to set up auditor", apigatewayv1alpha1.EndpointSubscription{}.APIVersion, apigatewayv1alpha1.EndpointSubscription{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apigatewayv1alpha1.EndpointSubscription{}).
...
func (r *EndpointSubscriptionReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for EndpointSubscription")
			return nil
		}
		...
		resourceList := &apigatewayv1alpha1.EndpointSubscriptionList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list EndpointSubscription")
			return nil
		}
