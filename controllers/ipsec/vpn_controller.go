...
// +kubebuilder:rbac:groups=ipsec.ibm.kubeform.com,resources=vpns,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ipsec.ibm.kubeform.com,resources=vpns/status,verbs=get;update;patch

func (r *VpnReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vpn", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Vpn")
...
func (r *VpnReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ipsecv1alpha1.Vpn{}); err != nil {
			klog.Error(err, "unable to set up auditor", ipsecv1alpha1.Vpn{}.APIVersion, ipsecv1alpha1.Vpn{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ipsecv1alpha1.Vpn{}).
