...
// ConfigEnvironmentReconciler reconciles a ConfigEnvironment object
type ConfigEnvironmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=app.ibm.kubeform.com,resources=configenvironments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=app.ibm.kubeform.com,resources=configenvironments/status,verbs=get;update;patch

func (r *ConfigEnvironmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("configenvironment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConfigEnvironment")
...
func (r *ConfigEnvironmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appv1alpha1.ConfigEnvironment{}); err != nil {
			klog.Error(err, "unable to set up auditor", appv1alpha1.ConfigEnvironment{}.APIVersion, appv1alpha1.ConfigEnvironment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appv1alpha1.ConfigEnvironment{}).
