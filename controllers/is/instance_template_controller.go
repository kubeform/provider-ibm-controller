...
// +kubebuilder:rbac:groups=is.ibm.kubeform.com,resources=instancetemplates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=is.ibm.kubeform.com,resources=instancetemplates/status,verbs=get;update;patch
...
func (r *InstanceTemplateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch InstanceTemplate")
...
func (r *InstanceTemplateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &isv1alpha1.InstanceTemplate{}); err != nil {
			klog.Error(err, "unable to set up auditor", isv1alpha1.InstanceTemplate{}.APIVersion, isv1alpha1.InstanceTemplate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&isv1alpha1.InstanceTemplate{}).
