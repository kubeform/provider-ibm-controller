...
// VirtualEndpointGatewayReconciler reconciles a VirtualEndpointGateway object
type VirtualEndpointGatewayReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=is.ibm.kubeform.com,resources=virtualendpointgateways,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=is.ibm.kubeform.com,resources=virtualendpointgateways/status,verbs=get;update;patch

func (r *VirtualEndpointGatewayReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("virtualendpointgateway", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VirtualEndpointGateway")
...
func (r *VirtualEndpointGatewayReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &isv1alpha1.VirtualEndpointGateway{}); err != nil {
			klog.Error(err, "unable to set up auditor", isv1alpha1.VirtualEndpointGateway{}.APIVersion, isv1alpha1.VirtualEndpointGateway{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&isv1alpha1.VirtualEndpointGateway{}).
