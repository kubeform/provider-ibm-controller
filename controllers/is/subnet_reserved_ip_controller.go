...
// SubnetReservedIPReconciler reconciles a SubnetReservedIP object
type SubnetReservedIPReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=is.ibm.kubeform.com,resources=subnetreservedips,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=is.ibm.kubeform.com,resources=subnetreservedips/status,verbs=get;update;patch

func (r *SubnetReservedIPReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("subnetreservedip", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SubnetReservedIP")
...
func (r *SubnetReservedIPReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &isv1alpha1.SubnetReservedIP{}); err != nil {
			klog.Error(err, "unable to set up auditor", isv1alpha1.SubnetReservedIP{}.APIVersion, isv1alpha1.SubnetReservedIP{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&isv1alpha1.SubnetReservedIP{}).
