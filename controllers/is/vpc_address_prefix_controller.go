...
// VpcAddressPrefixReconciler reconciles a VpcAddressPrefix object
type VpcAddressPrefixReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=is.ibm.kubeform.com,resources=vpcaddressprefixes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=is.ibm.kubeform.com,resources=vpcaddressprefixes/status,verbs=get;update;patch

func (r *VpcAddressPrefixReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vpcaddressprefix", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VpcAddressPrefix")
...
func (r *VpcAddressPrefixReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &isv1alpha1.VpcAddressPrefix{}); err != nil {
			klog.Error(err, "unable to set up auditor", isv1alpha1.VpcAddressPrefix{}.APIVersion, isv1alpha1.VpcAddressPrefix{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&isv1alpha1.VpcAddressPrefix{}).
