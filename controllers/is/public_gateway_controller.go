...
// PublicGatewayReconciler reconciles a PublicGateway object
...
// +kubebuilder:rbac:groups=is.ibm.kubeform.com,resources=publicgateways,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=is.ibm.kubeform.com,resources=publicgateways/status,verbs=get;update;patch

func (r *PublicGatewayReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("publicgateway", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PublicGateway")
...
func (r *PublicGatewayReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &isv1alpha1.PublicGateway{}); err != nil {
			klog.Error(err, "unable to set up auditor", isv1alpha1.PublicGateway{}.APIVersion, isv1alpha1.PublicGateway{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&isv1alpha1.PublicGateway{}).
