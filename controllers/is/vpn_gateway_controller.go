...
// +kubebuilder:rbac:groups=is.ibm.kubeform.com,resources=vpngateways,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=is.ibm.kubeform.com,resources=vpngateways/status,verbs=get;update;patch

func (r *VpnGatewayReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vpngateway", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VpnGateway")
...
func (r *VpnGatewayReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &isv1alpha1.VpnGateway{}); err != nil {
			klog.Error(err, "unable to set up auditor", isv1alpha1.VpnGateway{}.APIVersion, isv1alpha1.VpnGateway{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&isv1alpha1.VpnGateway{}).
