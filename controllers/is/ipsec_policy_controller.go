...
// +kubebuilder:rbac:groups=is.ibm.kubeform.com,resources=ipsecpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=is.ibm.kubeform.com,resources=ipsecpolicies/status,verbs=get;update;patch

func (r *IpsecPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ipsecpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IpsecPolicy")
...
func (r *IpsecPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &isv1alpha1.IpsecPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", isv1alpha1.IpsecPolicy{}.APIVersion, isv1alpha1.IpsecPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&isv1alpha1.IpsecPolicy{}).
