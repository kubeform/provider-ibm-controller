...
// +kubebuilder:rbac:groups=is.ibm.kubeform.com,resources=securitygrouprules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=is.ibm.kubeform.com,resources=securitygrouprules/status,verbs=get;update;patch

func (r *SecurityGroupRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("securitygrouprule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SecurityGroupRule")
...
func (r *SecurityGroupRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &isv1alpha1.SecurityGroupRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", isv1alpha1.SecurityGroupRule{}.APIVersion, isv1alpha1.SecurityGroupRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&isv1alpha1.SecurityGroupRule{}).
