...
// EvaultReconciler reconciles a Evault object
type EvaultReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=storage.ibm.kubeform.com,resources=evaults,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=storage.ibm.kubeform.com,resources=evaults/status,verbs=get;update;patch

func (r *EvaultReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("evault", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Evault")
...
func (r *EvaultReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &storagev1alpha1.Evault{}); err != nil {
			klog.Error(err, "unable to set up auditor", storagev1alpha1.Evault{}.APIVersion, storagev1alpha1.Evault{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&storagev1alpha1.Evault{}).
...
func (r *EvaultReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for Evault")
			return nil
		}
		...
		resourceList := &storagev1alpha1.EvaultList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list Evault")
			return nil
		}
