...
// BlockReconciler reconciles a Block object
...
// +kubebuilder:rbac:groups=storage.ibm.kubeform.com,resources=blocks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=storage.ibm.kubeform.com,resources=blocks/status,verbs=get;update;patch

func (r *BlockReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("block", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Block")
...
func (r *BlockReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &storagev1alpha1.Block{}); err != nil {
			klog.Error(err, "unable to set up auditor", storagev1alpha1.Block{}.APIVersion, storagev1alpha1.Block{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&storagev1alpha1.Block{}).
