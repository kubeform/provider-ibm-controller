...
// +kubebuilder:rbac:groups=object.ibm.kubeform.com,resources=storageaccounts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=object.ibm.kubeform.com,resources=storageaccounts/status,verbs=get;update;patch

func (r *StorageAccountReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("storageaccount", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch StorageAccount")
...
func (r *StorageAccountReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &objectv1alpha1.StorageAccount{}); err != nil {
			klog.Error(err, "unable to set up auditor", objectv1alpha1.StorageAccount{}.APIVersion, objectv1alpha1.StorageAccount{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&objectv1alpha1.StorageAccount{}).
