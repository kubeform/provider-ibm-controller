...
// KeyAliasReconciler reconciles a KeyAlias object
type KeyAliasReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=kms.ibm.kubeform.com,resources=keyalias,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kms.ibm.kubeform.com,resources=keyalias/status,verbs=get;update;patch

func (r *KeyAliasReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("keyalias", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch KeyAlias")
...
func (r *KeyAliasReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kmsv1alpha1.KeyAlias{}); err != nil {
			klog.Error(err, "unable to set up auditor", kmsv1alpha1.KeyAlias{}.APIVersion, kmsv1alpha1.KeyAlias{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kmsv1alpha1.KeyAlias{}).
