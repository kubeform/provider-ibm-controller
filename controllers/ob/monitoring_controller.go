...
// +kubebuilder:rbac:groups=ob.ibm.kubeform.com,resources=monitorings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ob.ibm.kubeform.com,resources=monitorings/status,verbs=get;update;patch
...
func (r *MonitoringReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Monitoring")
...
func (r *MonitoringReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &obv1alpha1.Monitoring{}); err != nil {
			klog.Error(err, "unable to set up auditor", obv1alpha1.Monitoring{}.APIVersion, obv1alpha1.Monitoring{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&obv1alpha1.Monitoring{}).
