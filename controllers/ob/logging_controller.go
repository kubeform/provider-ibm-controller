...
// +kubebuilder:rbac:groups=ob.ibm.kubeform.com,resources=loggings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ob.ibm.kubeform.com,resources=loggings/status,verbs=get;update;patch
...
func (r *LoggingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Logging")
...
func (r *LoggingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &obv1alpha1.Logging{}); err != nil {
			klog.Error(err, "unable to set up auditor", obv1alpha1.Logging{}.APIVersion, obv1alpha1.Logging{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&obv1alpha1.Logging{}).
