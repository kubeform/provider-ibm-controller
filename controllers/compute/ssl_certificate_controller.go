...
// SslCertificateReconciler reconciles a SslCertificate object
type SslCertificateReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=compute.ibm.kubeform.com,resources=sslcertificates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=compute.ibm.kubeform.com,resources=sslcertificates/status,verbs=get;update;patch

func (r *SslCertificateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sslcertificate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SslCertificate")
...
func (r *SslCertificateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &computev1alpha1.SslCertificate{}); err != nil {
			klog.Error(err, "unable to set up auditor", computev1alpha1.SslCertificate{}.APIVersion, computev1alpha1.SslCertificate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&computev1alpha1.SslCertificate{}).
...
func (r *SslCertificateReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for SslCertificate")
			return nil
		}
		...
		resourceList := &computev1alpha1.SslCertificateList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list SslCertificate")
			return nil
		}
