...
// ProvisioningHookReconciler reconciles a ProvisioningHook object
type ProvisioningHookReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=compute.ibm.kubeform.com,resources=provisioninghooks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=compute.ibm.kubeform.com,resources=provisioninghooks/status,verbs=get;update;patch

func (r *ProvisioningHookReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("provisioninghook", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProvisioningHook")
...
func (r *ProvisioningHookReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &computev1alpha1.ProvisioningHook{}); err != nil {
			klog.Error(err, "unable to set up auditor", computev1alpha1.ProvisioningHook{}.APIVersion, computev1alpha1.ProvisioningHook{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&computev1alpha1.ProvisioningHook{}).
