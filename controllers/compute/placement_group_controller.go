...
// PlacementGroupReconciler reconciles a PlacementGroup object
...
// +kubebuilder:rbac:groups=compute.ibm.kubeform.com,resources=placementgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=compute.ibm.kubeform.com,resources=placementgroups/status,verbs=get;update;patch

func (r *PlacementGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("placementgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PlacementGroup")
...
func (r *PlacementGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &computev1alpha1.PlacementGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", computev1alpha1.PlacementGroup{}.APIVersion, computev1alpha1.PlacementGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&computev1alpha1.PlacementGroup{}).
