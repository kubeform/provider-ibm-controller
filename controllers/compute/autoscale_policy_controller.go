...
// AutoscalePolicyReconciler reconciles a AutoscalePolicy object
type AutoscalePolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=compute.ibm.kubeform.com,resources=autoscalepolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=compute.ibm.kubeform.com,resources=autoscalepolicies/status,verbs=get;update;patch

func (r *AutoscalePolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("autoscalepolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AutoscalePolicy")
...
func (r *AutoscalePolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &computev1alpha1.AutoscalePolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", computev1alpha1.AutoscalePolicy{}.APIVersion, computev1alpha1.AutoscalePolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&computev1alpha1.AutoscalePolicy{}).
