...
// VolumeAttachReconciler reconciles a VolumeAttach object
type VolumeAttachReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=pi.ibm.kubeform.com,resources=volumeattaches,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=pi.ibm.kubeform.com,resources=volumeattaches/status,verbs=get;update;patch

func (r *VolumeAttachReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("volumeattach", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VolumeAttach")
...
func (r *VolumeAttachReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &piv1alpha1.VolumeAttach{}); err != nil {
			klog.Error(err, "unable to set up auditor", piv1alpha1.VolumeAttach{}.APIVersion, piv1alpha1.VolumeAttach{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&piv1alpha1.VolumeAttach{}).
