...
// OperationsReconciler reconciles a Operations object
...
// +kubebuilder:rbac:groups=pi.ibm.kubeform.com,resources=operations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=pi.ibm.kubeform.com,resources=operations/status,verbs=get;update;patch

func (r *OperationsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("operations", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Operations")
...
func (r *OperationsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &piv1alpha1.Operations{}); err != nil {
			klog.Error(err, "unable to set up auditor", piv1alpha1.Operations{}.APIVersion, piv1alpha1.Operations{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&piv1alpha1.Operations{}).
