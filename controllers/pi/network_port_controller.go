...
// NetworkPortReconciler reconciles a NetworkPort object
type NetworkPortReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=pi.ibm.kubeform.com,resources=networkports,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=pi.ibm.kubeform.com,resources=networkports/status,verbs=get;update;patch

func (r *NetworkPortReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("networkport", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NetworkPort")
...
func (r *NetworkPortReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &piv1alpha1.NetworkPort{}); err != nil {
			klog.Error(err, "unable to set up auditor", piv1alpha1.NetworkPort{}.APIVersion, piv1alpha1.NetworkPort{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&piv1alpha1.NetworkPort{}).
