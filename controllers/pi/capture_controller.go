...
// +kubebuilder:rbac:groups=pi.ibm.kubeform.com,resources=captures,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=pi.ibm.kubeform.com,resources=captures/status,verbs=get;update;patch

func (r *CaptureReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("capture", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Capture")
...
func (r *CaptureReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &piv1alpha1.Capture{}); err != nil {
			klog.Error(err, "unable to set up auditor", piv1alpha1.Capture{}.APIVersion, piv1alpha1.Capture{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&piv1alpha1.Capture{}).
