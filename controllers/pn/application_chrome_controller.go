...
// ApplicationChromeReconciler reconciles a ApplicationChrome object
type ApplicationChromeReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=pn.ibm.kubeform.com,resources=applicationchromes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=pn.ibm.kubeform.com,resources=applicationchromes/status,verbs=get;update;patch

func (r *ApplicationChromeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("applicationchrome", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ApplicationChrome")
...
func (r *ApplicationChromeReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &pnv1alpha1.ApplicationChrome{}); err != nil {
			klog.Error(err, "unable to set up auditor", pnv1alpha1.ApplicationChrome{}.APIVersion, pnv1alpha1.ApplicationChrome{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&pnv1alpha1.ApplicationChrome{}).
