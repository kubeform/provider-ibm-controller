...
// +kubebuilder:rbac:groups=database.ibm.kubeform.com,resources=databases,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.ibm.kubeform.com,resources=databases/status,verbs=get;update;patch
...
func (r *DatabaseReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.Database{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.Database{}.APIVersion, databasev1alpha1.Database{}.Kind)
			return err
		}
	}
...
func (r *DatabaseReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for Database")
			return nil
		}
		...
		resourceList := &databasev1alpha1.DatabaseList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list Database")
			return nil
		}
