...
// VlanFirewallReconciler reconciles a VlanFirewall object
type VlanFirewallReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=multi.ibm.kubeform.com,resources=vlanfirewalls,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=multi.ibm.kubeform.com,resources=vlanfirewalls/status,verbs=get;update;patch

func (r *VlanFirewallReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vlanfirewall", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VlanFirewall")
...
func (r *VlanFirewallReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &multiv1alpha1.VlanFirewall{}); err != nil {
			klog.Error(err, "unable to set up auditor", multiv1alpha1.VlanFirewall{}.APIVersion, multiv1alpha1.VlanFirewall{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&multiv1alpha1.VlanFirewall{}).
...
func (r *VlanFirewallReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for VlanFirewall")
			return nil
		}
		...
		resourceList := &multiv1alpha1.VlanFirewallList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list VlanFirewall")
			return nil
		}
