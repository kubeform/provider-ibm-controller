...
// PermittedNetworkReconciler reconciles a PermittedNetwork object
type PermittedNetworkReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dns.ibm.kubeform.com,resources=permittednetworks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dns.ibm.kubeform.com,resources=permittednetworks/status,verbs=get;update;patch

func (r *PermittedNetworkReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("permittednetwork", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PermittedNetwork")
...
func (r *PermittedNetworkReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dnsv1alpha1.PermittedNetwork{}); err != nil {
			klog.Error(err, "unable to set up auditor", dnsv1alpha1.PermittedNetwork{}.APIVersion, dnsv1alpha1.PermittedNetwork{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dnsv1alpha1.PermittedNetwork{}).
