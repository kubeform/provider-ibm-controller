...
// GlbPoolReconciler reconciles a GlbPool object
type GlbPoolReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dns.ibm.kubeform.com,resources=glbpools,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dns.ibm.kubeform.com,resources=glbpools/status,verbs=get;update;patch

func (r *GlbPoolReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("glbpool", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GlbPool")
...
func (r *GlbPoolReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dnsv1alpha1.GlbPool{}); err != nil {
			klog.Error(err, "unable to set up auditor", dnsv1alpha1.GlbPool{}.APIVersion, dnsv1alpha1.GlbPool{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dnsv1alpha1.GlbPool{}).
