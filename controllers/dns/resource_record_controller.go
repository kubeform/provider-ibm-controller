...
// +kubebuilder:rbac:groups=dns.ibm.kubeform.com,resources=resourcerecords,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dns.ibm.kubeform.com,resources=resourcerecords/status,verbs=get;update;patch

func (r *ResourceRecordReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("resourcerecord", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ResourceRecord")
...
func (r *ResourceRecordReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dnsv1alpha1.ResourceRecord{}); err != nil {
			klog.Error(err, "unable to set up auditor", dnsv1alpha1.ResourceRecord{}.APIVersion, dnsv1alpha1.ResourceRecord{}.Kind)
			return err
		}
	}
