...
// SecondaryReconciler reconciles a Secondary object
...
// +kubebuilder:rbac:groups=dns.ibm.kubeform.com,resources=secondaries,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dns.ibm.kubeform.com,resources=secondaries/status,verbs=get;update;patch

func (r *SecondaryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("secondary", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Secondary")
...
func (r *SecondaryReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dnsv1alpha1.Secondary{}); err != nil {
			klog.Error(err, "unable to set up auditor", dnsv1alpha1.Secondary{}.APIVersion, dnsv1alpha1.Secondary{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dnsv1alpha1.Secondary{}).
