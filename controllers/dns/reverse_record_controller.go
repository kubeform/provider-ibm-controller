...
// ReverseRecordReconciler reconciles a ReverseRecord object
type ReverseRecordReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dns.ibm.kubeform.com,resources=reverserecords,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dns.ibm.kubeform.com,resources=reverserecords/status,verbs=get;update;patch

func (r *ReverseRecordReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("reverserecord", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ReverseRecord")
...
func (r *ReverseRecordReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dnsv1alpha1.ReverseRecord{}); err != nil {
			klog.Error(err, "unable to set up auditor", dnsv1alpha1.ReverseRecord{}.APIVersion, dnsv1alpha1.ReverseRecord{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dnsv1alpha1.ReverseRecord{}).
