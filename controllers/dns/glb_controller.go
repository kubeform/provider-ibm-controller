...
// GlbReconciler reconciles a Glb object
type GlbReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dns.ibm.kubeform.com,resources=glbs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dns.ibm.kubeform.com,resources=glbs/status,verbs=get;update;patch

func (r *GlbReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("glb", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Glb")
...
func (r *GlbReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dnsv1alpha1.Glb{}); err != nil {
			klog.Error(err, "unable to set up auditor", dnsv1alpha1.Glb{}.APIVersion, dnsv1alpha1.Glb{}.Kind)
			return err
		}
	}
