...
// ManagerImportReconciler reconciles a ManagerImport object
type ManagerImportReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=certificate.ibm.kubeform.com,resources=managerimports,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=certificate.ibm.kubeform.com,resources=managerimports/status,verbs=get;update;patch

func (r *ManagerImportReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("managerimport", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ManagerImport")
...
func (r *ManagerImportReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &certificatev1alpha1.ManagerImport{}); err != nil {
			klog.Error(err, "unable to set up auditor", certificatev1alpha1.ManagerImport{}.APIVersion, certificatev1alpha1.ManagerImport{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&certificatev1alpha1.ManagerImport{}).
