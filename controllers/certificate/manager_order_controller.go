...
// ManagerOrderReconciler reconciles a ManagerOrder object
type ManagerOrderReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=certificate.ibm.kubeform.com,resources=managerorders,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=certificate.ibm.kubeform.com,resources=managerorders/status,verbs=get;update;patch

func (r *ManagerOrderReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("managerorder", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ManagerOrder")
...
func (r *ManagerOrderReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &certificatev1alpha1.ManagerOrder{}); err != nil {
			klog.Error(err, "unable to set up auditor", certificatev1alpha1.ManagerOrder{}.APIVersion, certificatev1alpha1.ManagerOrder{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&certificatev1alpha1.ManagerOrder{}).
