...
// VlanSpanningReconciler reconciles a VlanSpanning object
type VlanSpanningReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=network.ibm.kubeform.com,resources=vlanspannings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=network.ibm.kubeform.com,resources=vlanspannings/status,verbs=get;update;patch

func (r *VlanSpanningReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vlanspanning", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VlanSpanning")
...
func (r *VlanSpanningReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &networkv1alpha1.VlanSpanning{}); err != nil {
			klog.Error(err, "unable to set up auditor", networkv1alpha1.VlanSpanning{}.APIVersion, networkv1alpha1.VlanSpanning{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&networkv1alpha1.VlanSpanning{}).
