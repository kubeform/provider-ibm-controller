...
// StreamsTopicReconciler reconciles a StreamsTopic object
type StreamsTopicReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=event.ibm.kubeform.com,resources=streamstopics,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=event.ibm.kubeform.com,resources=streamstopics/status,verbs=get;update;patch

func (r *StreamsTopicReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("streamstopic", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch StreamsTopic")
...
func (r *StreamsTopicReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &eventv1alpha1.StreamsTopic{}); err != nil {
			klog.Error(err, "unable to set up auditor", eventv1alpha1.StreamsTopic{}.APIVersion, eventv1alpha1.StreamsTopic{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&eventv1alpha1.StreamsTopic{}).
