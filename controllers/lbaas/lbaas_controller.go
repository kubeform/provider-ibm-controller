...
// LbaasReconciler reconciles a Lbaas object
type LbaasReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=lbaas.ibm.kubeform.com,resources=lbaas,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=lbaas.ibm.kubeform.com,resources=lbaas/status,verbs=get;update;patch

func (r *LbaasReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("lbaas", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Lbaas")
...
func (r *LbaasReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &lbaasv1alpha1.Lbaas{}); err != nil {
			klog.Error(err, "unable to set up auditor", lbaasv1alpha1.Lbaas{}.APIVersion, lbaasv1alpha1.Lbaas{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&lbaasv1alpha1.Lbaas{}).
