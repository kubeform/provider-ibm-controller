...
// +kubebuilder:rbac:groups=lbaas.ibm.kubeform.com,resources=healthmonitors,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=lbaas.ibm.kubeform.com,resources=healthmonitors/status,verbs=get;update;patch

func (r *HealthMonitorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("healthmonitor", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HealthMonitor")
...
func (r *HealthMonitorReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &lbaasv1alpha1.HealthMonitor{}); err != nil {
			klog.Error(err, "unable to set up auditor", lbaasv1alpha1.HealthMonitor{}.APIVersion, lbaasv1alpha1.HealthMonitor{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&lbaasv1alpha1.HealthMonitor{}).
