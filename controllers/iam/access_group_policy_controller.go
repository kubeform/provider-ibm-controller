...
// AccessGroupPolicyReconciler reconciles a AccessGroupPolicy object
type AccessGroupPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iam.ibm.kubeform.com,resources=accessgrouppolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iam.ibm.kubeform.com,resources=accessgrouppolicies/status,verbs=get;update;patch

func (r *AccessGroupPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("accessgrouppolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AccessGroupPolicy")
...
func (r *AccessGroupPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iamv1alpha1.AccessGroupPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", iamv1alpha1.AccessGroupPolicy{}.APIVersion, iamv1alpha1.AccessGroupPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iamv1alpha1.AccessGroupPolicy{}).
