...
// +kubebuilder:rbac:groups=iam.ibm.kubeform.com,resources=userpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iam.ibm.kubeform.com,resources=userpolicies/status,verbs=get;update;patch

func (r *UserPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("userpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch UserPolicy")
...
func (r *UserPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iamv1alpha1.UserPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", iamv1alpha1.UserPolicy{}.APIVersion, iamv1alpha1.UserPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iamv1alpha1.UserPolicy{}).
