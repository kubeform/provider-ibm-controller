...
// UserSettingsReconciler reconciles a UserSettings object
type UserSettingsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iam.ibm.kubeform.com,resources=usersettings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iam.ibm.kubeform.com,resources=usersettings/status,verbs=get;update;patch

func (r *UserSettingsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("usersettings", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch UserSettings")
...
func (r *UserSettingsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iamv1alpha1.UserSettings{}); err != nil {
			klog.Error(err, "unable to set up auditor", iamv1alpha1.UserSettings{}.APIVersion, iamv1alpha1.UserSettings{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iamv1alpha1.UserSettings{}).
