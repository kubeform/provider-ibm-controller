...
// AlbCertReconciler reconciles a AlbCert object
type AlbCertReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=container.ibm.kubeform.com,resources=albcerts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=container.ibm.kubeform.com,resources=albcerts/status,verbs=get;update;patch

func (r *AlbCertReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("albcert", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AlbCert")
...
func (r *AlbCertReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &containerv1alpha1.AlbCert{}); err != nil {
			klog.Error(err, "unable to set up auditor", containerv1alpha1.AlbCert{}.APIVersion, containerv1alpha1.AlbCert{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&containerv1alpha1.AlbCert{}).
