...
// +kubebuilder:rbac:groups=container.ibm.kubeform.com,resources=clusterfeatures,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=container.ibm.kubeform.com,resources=clusterfeatures/status,verbs=get;update;patch

func (r *ClusterFeatureReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("clusterfeature", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ClusterFeature")
...
func (r *ClusterFeatureReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &containerv1alpha1.ClusterFeature{}); err != nil {
			klog.Error(err, "unable to set up auditor", containerv1alpha1.ClusterFeature{}.APIVersion, containerv1alpha1.ClusterFeature{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&containerv1alpha1.ClusterFeature{}).
