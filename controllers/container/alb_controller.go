...
// AlbReconciler reconciles a Alb object
...
// +kubebuilder:rbac:groups=container.ibm.kubeform.com,resources=albs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=container.ibm.kubeform.com,resources=albs/status,verbs=get;update;patch

func (r *AlbReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("alb", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Alb")
...
func (r *AlbReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &containerv1alpha1.Alb{}); err != nil {
			klog.Error(err, "unable to set up auditor", containerv1alpha1.Alb{}.APIVersion, containerv1alpha1.Alb{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&containerv1alpha1.Alb{}).
