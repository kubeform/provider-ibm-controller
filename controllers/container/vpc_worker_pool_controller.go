...
// VpcWorkerPoolReconciler reconciles a VpcWorkerPool object
type VpcWorkerPoolReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=container.ibm.kubeform.com,resources=vpcworkerpools,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=container.ibm.kubeform.com,resources=vpcworkerpools/status,verbs=get;update;patch

func (r *VpcWorkerPoolReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vpcworkerpool", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VpcWorkerPool")
...
func (r *VpcWorkerPoolReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &containerv1alpha1.VpcWorkerPool{}); err != nil {
			klog.Error(err, "unable to set up auditor", containerv1alpha1.VpcWorkerPool{}.APIVersion, containerv1alpha1.VpcWorkerPool{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&containerv1alpha1.VpcWorkerPool{}).
