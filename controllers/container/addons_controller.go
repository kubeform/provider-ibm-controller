...
// +kubebuilder:rbac:groups=container.ibm.kubeform.com,resources=addons,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=container.ibm.kubeform.com,resources=addons/status,verbs=get;update;patch
...
func (r *AddonsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Addons")
...
func (r *AddonsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &containerv1alpha1.Addons{}); err != nil {
			klog.Error(err, "unable to set up auditor", containerv1alpha1.Addons{}.APIVersion, containerv1alpha1.Addons{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&containerv1alpha1.Addons{}).
