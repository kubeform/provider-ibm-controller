...
// VpcClusterReconciler reconciles a VpcCluster object
type VpcClusterReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=container.ibm.kubeform.com,resources=vpcclusters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=container.ibm.kubeform.com,resources=vpcclusters/status,verbs=get;update;patch

func (r *VpcClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vpccluster", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VpcCluster")
...
func (r *VpcClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &containerv1alpha1.VpcCluster{}); err != nil {
			klog.Error(err, "unable to set up auditor", containerv1alpha1.VpcCluster{}.APIVersion, containerv1alpha1.VpcCluster{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&containerv1alpha1.VpcCluster{}).
...
func (r *VpcClusterReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for VpcCluster")
			return nil
		}
		...
		resourceList := &containerv1alpha1.VpcClusterList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list VpcCluster")
			return nil
		}
