...
// CertificateUploadReconciler reconciles a CertificateUpload object
...
// +kubebuilder:rbac:groups=cis.ibm.kubeform.com,resources=certificateuploads,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cis.ibm.kubeform.com,resources=certificateuploads/status,verbs=get;update;patch

func (r *CertificateUploadReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("certificateupload", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CertificateUpload")
...
func (r *CertificateUploadReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cisv1alpha1.CertificateUpload{}); err != nil {
			klog.Error(err, "unable to set up auditor", cisv1alpha1.CertificateUpload{}.APIVersion, cisv1alpha1.CertificateUpload{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cisv1alpha1.CertificateUpload{}).
...
func (r *CertificateUploadReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for CertificateUpload")
			return nil
		}
		...
		resourceList := &cisv1alpha1.CertificateUploadList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list CertificateUpload")
			return nil
		}
