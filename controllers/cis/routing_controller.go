...
// RoutingReconciler reconciles a Routing object
...
// +kubebuilder:rbac:groups=cis.ibm.kubeform.com,resources=routings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cis.ibm.kubeform.com,resources=routings/status,verbs=get;update;patch

func (r *RoutingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("routing", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Routing")
...
func (r *RoutingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cisv1alpha1.Routing{}); err != nil {
			klog.Error(err, "unable to set up auditor", cisv1alpha1.Routing{}.APIVersion, cisv1alpha1.Routing{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cisv1alpha1.Routing{}).
