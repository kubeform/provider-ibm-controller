...
// PageRuleReconciler reconciles a PageRule object
type PageRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cis.ibm.kubeform.com,resources=pagerules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cis.ibm.kubeform.com,resources=pagerules/status,verbs=get;update;patch

func (r *PageRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("pagerule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PageRule")
...
func (r *PageRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cisv1alpha1.PageRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", cisv1alpha1.PageRule{}.APIVersion, cisv1alpha1.PageRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cisv1alpha1.PageRule{}).
