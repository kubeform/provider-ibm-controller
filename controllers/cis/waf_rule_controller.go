...
// WafRuleReconciler reconciles a WafRule object
type WafRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cis.ibm.kubeform.com,resources=wafrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cis.ibm.kubeform.com,resources=wafrules/status,verbs=get;update;patch

func (r *WafRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("wafrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch WafRule")
...
func (r *WafRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cisv1alpha1.WafRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", cisv1alpha1.WafRule{}.APIVersion, cisv1alpha1.WafRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cisv1alpha1.WafRule{}).
