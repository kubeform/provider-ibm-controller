...
// EdgeFunctionsTriggerReconciler reconciles a EdgeFunctionsTrigger object
type EdgeFunctionsTriggerReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cis.ibm.kubeform.com,resources=edgefunctionstriggers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cis.ibm.kubeform.com,resources=edgefunctionstriggers/status,verbs=get;update;patch

func (r *EdgeFunctionsTriggerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("edgefunctionstrigger", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EdgeFunctionsTrigger")
...
func (r *EdgeFunctionsTriggerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cisv1alpha1.EdgeFunctionsTrigger{}); err != nil {
			klog.Error(err, "unable to set up auditor", cisv1alpha1.EdgeFunctionsTrigger{}.APIVersion, cisv1alpha1.EdgeFunctionsTrigger{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cisv1alpha1.EdgeFunctionsTrigger{}).
