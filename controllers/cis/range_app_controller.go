...
// RangeAppReconciler reconciles a RangeApp object
type RangeAppReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cis.ibm.kubeform.com,resources=rangeapps,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cis.ibm.kubeform.com,resources=rangeapps/status,verbs=get;update;patch

func (r *RangeAppReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("rangeapp", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RangeApp")
...
func (r *RangeAppReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cisv1alpha1.RangeApp{}); err != nil {
			klog.Error(err, "unable to set up auditor", cisv1alpha1.RangeApp{}.APIVersion, cisv1alpha1.RangeApp{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cisv1alpha1.RangeApp{}).
