...
// +kubebuilder:rbac:groups=cr.ibm.kubeform.com,resources=retentionpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cr.ibm.kubeform.com,resources=retentionpolicies/status,verbs=get;update;patch
...
func (r *RetentionPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RetentionPolicy")
...
func (r *RetentionPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &crv1alpha1.RetentionPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", crv1alpha1.RetentionPolicy{}.APIVersion, crv1alpha1.RetentionPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&crv1alpha1.RetentionPolicy{}).
