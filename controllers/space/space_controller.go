...
// +kubebuilder:rbac:groups=space.ibm.kubeform.com,resources=spaces,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=space.ibm.kubeform.com,resources=spaces/status,verbs=get;update;patch
...
func (r *SpaceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Space")
...
func (r *SpaceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &spacev1alpha1.Space{}); err != nil {
			klog.Error(err, "unable to set up auditor", spacev1alpha1.Space{}.APIVersion, spacev1alpha1.Space{}.Kind)
			return err
		}
	}
