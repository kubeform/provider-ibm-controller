...
// ProviderGatewayReconciler reconciles a ProviderGateway object
type ProviderGatewayReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dl.ibm.kubeform.com,resources=providergateways,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dl.ibm.kubeform.com,resources=providergateways/status,verbs=get;update;patch

func (r *ProviderGatewayReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("providergateway", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProviderGateway")
...
func (r *ProviderGatewayReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dlv1alpha1.ProviderGateway{}); err != nil {
			klog.Error(err, "unable to set up auditor", dlv1alpha1.ProviderGateway{}.APIVersion, dlv1alpha1.ProviderGateway{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dlv1alpha1.ProviderGateway{}).
