...
// +kubebuilder:rbac:groups=cm.ibm.kubeform.com,resources=catalogs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cm.ibm.kubeform.com,resources=catalogs/status,verbs=get;update;patch
...
func (r *CatalogReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Catalog")
...
func (r *CatalogReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cmv1alpha1.Catalog{}); err != nil {
			klog.Error(err, "unable to set up auditor", cmv1alpha1.Catalog{}.APIVersion, cmv1alpha1.Catalog{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cmv1alpha1.Catalog{}).
