...
// OfferingInstanceReconciler reconciles a OfferingInstance object
type OfferingInstanceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cm.ibm.kubeform.com,resources=offeringinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cm.ibm.kubeform.com,resources=offeringinstances/status,verbs=get;update;patch

func (r *OfferingInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("offeringinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch OfferingInstance")
...
func (r *OfferingInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cmv1alpha1.OfferingInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", cmv1alpha1.OfferingInstance{}.APIVersion, cmv1alpha1.OfferingInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cmv1alpha1.OfferingInstance{}).
