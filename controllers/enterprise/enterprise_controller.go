...
// +kubebuilder:rbac:groups=enterprise.ibm.kubeform.com,resources=enterprises,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=enterprise.ibm.kubeform.com,resources=enterprises/status,verbs=get;update;patch

func (r *EnterpriseReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("enterprise", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Enterprise")
...
func (r *EnterpriseReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &enterprisev1alpha1.Enterprise{}); err != nil {
			klog.Error(err, "unable to set up auditor", enterprisev1alpha1.Enterprise{}.APIVersion, enterprisev1alpha1.Enterprise{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&enterprisev1alpha1.Enterprise{}).
