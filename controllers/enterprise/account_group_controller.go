...
// AccountGroupReconciler reconciles a AccountGroup object
type AccountGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=enterprise.ibm.kubeform.com,resources=accountgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=enterprise.ibm.kubeform.com,resources=accountgroups/status,verbs=get;update;patch

func (r *AccountGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("accountgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AccountGroup")
...
func (r *AccountGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &enterprisev1alpha1.AccountGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", enterprisev1alpha1.AccountGroup{}.APIVersion, enterprisev1alpha1.AccountGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&enterprisev1alpha1.AccountGroup{}).
