...
// HpcsReconciler reconciles a Hpcs object
type HpcsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=hpcs.ibm.kubeform.com,resources=hpcs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=hpcs.ibm.kubeform.com,resources=hpcs/status,verbs=get;update;patch

func (r *HpcsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("hpcs", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Hpcs")
...
func (r *HpcsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &hpcsv1alpha1.Hpcs{}); err != nil {
			klog.Error(err, "unable to set up auditor", hpcsv1alpha1.Hpcs{}.APIVersion, hpcsv1alpha1.Hpcs{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&hpcsv1alpha1.Hpcs{}).
