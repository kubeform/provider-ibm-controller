...
// FirewallSharedReconciler reconciles a FirewallShared object
type FirewallSharedReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=hardware.ibm.kubeform.com,resources=firewallshareds,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=hardware.ibm.kubeform.com,resources=firewallshareds/status,verbs=get;update;patch

func (r *FirewallSharedReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("firewallshared", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FirewallShared")
...
func (r *FirewallSharedReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &hardwarev1alpha1.FirewallShared{}); err != nil {
			klog.Error(err, "unable to set up auditor", hardwarev1alpha1.FirewallShared{}.APIVersion, hardwarev1alpha1.FirewallShared{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&hardwarev1alpha1.FirewallShared{}).
