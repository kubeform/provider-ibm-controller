...
// +kubebuilder:rbac:groups=function.ibm.kubeform.com,resources=packages,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=function.ibm.kubeform.com,resources=packages/status,verbs=get;update;patch

func (r *PackageReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("package", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Package")
...
func (r *PackageReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &functionv1alpha1.Package{}); err != nil {
			klog.Error(err, "unable to set up auditor", functionv1alpha1.Package{}.APIVersion, functionv1alpha1.Package{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&functionv1alpha1.Package{}).
